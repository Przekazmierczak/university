...
int main() {
    ...
    Matrix nonparallel = matrixMultiplication(matrix1, matrix2);
    auto end2 = std::chrono::high_resolution_clock::now();
    auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start2);
    std::cout << "Time nonpara multi: " << duration2.count() << " ms" << std::endl;

    auto start3 = std::chrono::high_resolution_clock::now();
    nonparallel.saveToFile("result.txt");
    auto end3 = std::chrono::high_resolution_clock::now();
    auto duration3 = std::chrono::duration_cast<std::chrono::milliseconds>(end3 - start3);
    std::cout << "Time write: " << duration3.count() << " ms" << std::endl;

    std::cout << "Total nonparallel: " << duration1.count() + duration2.count() + duration3.count() << " ms" << std::endl;
