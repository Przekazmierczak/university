...
bool CRayTrace::rayTrace(const CScene& scene, CRay& ray, COutput& out) {
    ...
    for (auto light : scene.lightList) {
        out.col = out.col + light.color * hitObject->matAmbient;

        glm::vec3 n = hitObject->normal(p);

        glm::vec3 L = glm::normalize(light.pos - p);

        float cosAngle = glm::dot(n, L);

        if (cosAngle > 0.001f) {
            out.col = out.col + light.color * hitObject->matDiffuse * cosAngle;
            glm::vec3 h = glm::normalize(L - ray.dir);
            float cosBeta = glm::dot(n, h);

            if (cosBeta > 0.001f) {
                out.col = out.col + light.color * hitObject->matSpecular * std::pow(cosBeta, hitObject->matShininess);
            }
        }
    }
